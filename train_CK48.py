...
parser = argparse.ArgumentParser("CK48")
# parser.add_argument('--data', type=str, default='../data', help='location of the data corpus')
parser.add_argument('--batch_size', type=int, default=64, help='batch size')
parser.add_argument('--learning_rate', type=float, default=0.01794073, help='init learning rate')
...
args.save = './CK-log/eval-{}-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"), str(uuid.uuid4()))
...
def main():
  ...
  test_acc_all, best_test_acc_all = [], []
  for i in range(10):
    fold = i + 1
    logging.info("This is the %d fold\n", fold)
    ...
    train_queue, test_queue = GetCK(args, fold)
    ...
    for epoch in range(args.epochs):
      ...
      model.drop_path_prob = args.drop_path_prob * (epoch + 223) / (600)  # args.epochs
  ...
  logging.info('avg_test_acc %f\tavg_best_test_acc %f', sum(test_acc_all)/10, sum(best_test_acc_all)/10)
